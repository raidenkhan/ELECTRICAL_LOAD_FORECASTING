...
```mermaid
flowchart LR
    A["Historical Data<br/>(168h demand + features)"] --> B["Normalization<br/>(z-score via recent stats)"]
    B --> C["DLinear Model<br/>(single, recent training)"]
    C --> D["TIDE Corrector<br/>(EMA bias estimate)"]
    D --> E["24h Forecast<br/>(denormalized)"]

    style A fill:#e8f4f8 color:#000
    style C fill:#4C72B0 color:#fff
    style D fill:#DD8452 color:#fff
    style E fill:#55A868 color:#fff
```
...
A useful intuition: think of the forecast error as a sound wave. A spectral analysis decomposes it into fast and slow components. The slow-moving part (bias) is like a low hum that persists for days --- it has energy at long timescales. The fast part (noise) is like static crackling --- it jumps around hour to hour but cancels out over time. The bias hum is predictable (today's error tells you about tomorrow's), while the static is not. This structure is what makes online correction possible: if most of the error power is in the slow drift rather than the fast noise, a simple smoothing filter can extract the bias by averaging out the jitter.
...
```mermaid
flowchart TD
    A["Normalized forecast<br/>from DLinear model"] --> B["Compute error<br/>e = ŷ - y<br/>(using observed actual)"]
    B --> C["Update EMA<br/>b ← α·e + (1-α)·b<br/>α = 0.3"]
    C --> D["Store bias b<br/>for next step"]
    D --> E["Next forecast arrives<br/>Apply correction<br/>ŷ' = ŷ - b"]
    E --> F["Denormalize &<br/>emit 24h forecast"]
    F -->|next day| A

    style A fill:#4C72B0,color:#fff
    style C fill:#55A868,color:#fff
