...
\usepackage[top=0.75in,bottom=0.75in,left=0.68in,right=0.68in]{geometry}
\usepackage{times}
...
% Spacing optimization for page budget (target < 8 pages)
...
\author{Feynman Research Group \\ Department of Engineering, Advanced Energy Analytics Division \\ {\tt\small research@group.org}}
\date{}
...
\begin{figure}[t]
    \centering
    \includegraphics[width=0.9\columnwidth]{figures/fig1_load_growth.png}
...
\begin{figure}[t]
    \centering
    \includegraphics[width=\columnwidth]{figures/Methodology_1.png}
...
\begin{enumerate}
    ...
    \item We analyze the residual structure of load forecasts, demonstrating that distribution shift manifests as a low-frequency, persistent bias rather than random noise.
...
\begin{figure}[h]
    \centering
    \includegraphics[width=0.75\columnwidth]{figures/Folds.png}
...
\begin{figure}[h]
    \centering
    \includegraphics[width=0.8\columnwidth]{figures/fig6_degradation.png}
...
\begin{figure}[h]
    \centering
    \includegraphics[width=0.8\columnwidth]{figures/fig2_autocorrelation.png}
...
\begin{figure}[t]
    \centering
    \includegraphics[width=0.8\columnwidth]{figures/TIDA.png}
...
\begin{figure}[h]
    \centering
    \includegraphics[width=0.8\columnwidth]{figures/fig3_main_result.png}
    \caption{Fold-by-fold MAE comparison of baseline DLinear (orange) vs DLinear + TIDE (blue).}
...
\begin{figure}[h]
    \centering
    \includegraphics[width=0.8\columnwidth]{figures/fig4_corrector_comparison.png}
...
\begin{figure}[h]
    \centering
    \includegraphics[width=0.8\columnwidth]{figures/fig5_alpha_sensitivity.png}
...
\begin{figure}[h]
    \centering
    \includegraphics[width=0.8\columnwidth]{figures/fig7_sobolev.png}
...
The high efficacy of the TIDE mechanism is explained by the physical and spectral characteristics of load drift on growing grids. The grid's compound growth rate of 6–14% annually results in a fast drift velocity, which causes parameters trained on historical scales to fall behind within days of deployment. TIDE tracks and corrects this drift at the output level with an update cycle of 2–3 days ($\alpha \ge 0.3$).

Crucially, TIDE's operation in normalized $z$-score space ensures scale invariance. Because the grid's demand scale doubled over the study period, a corrector operating in absolute Megawatts would over-correct early years and under-correct later years. Normalization allows TIDE to scale its corrections dynamically.

From a signal processing perspective, the forecast error is composed of a low-frequency drift signal (the ``bias hum'') and high-frequency noise (load-shedding transients, metering faults). The EMA acts as an online low-pass filter (an infinite impulse response filter with a pole at $z = 1 - \alpha$). This filter smooths out high-frequency, zero-mean noise and isolates the persistent, low-frequency drift signal, preventing the corrector from over-reacting to daily fluctuations.
...
These findings suggest that output-level correction is more robust than parameter-level adaptation under conditions of rapid, non-stationary drift.
...
\end{itemize}
Future work will investigate extending the feedback loop to track and correct higher-order moments (variance and covariance) in normalized space.

\section{Conclusion}
\label{sec:conclusion}
Short-term load forecasting on rapidly growing electricity grids suffers from systematic under-forecasting bias between retraining cycles. We proposed TIDE, a zero-parameter online bias correction mechanism operating in normalized $z$-score space. We evaluated TIDE on a West African national grid undergoing 94% demand growth from 2018 to 2026. TIDE reduced the mean fold MAE of the DLinear baseline by 19.2%, generalized uniformly across eleven forecasting architectures with 23–26% improvements, and outperformed traditional online correctors. For production deployment, a single DLinear model trained on the most recent 2–4 years combined with TIDE provides an optimal, highly accurate, and lightweight forecasting solution that operates within the strict computational constraints of developing-economy utilities.
